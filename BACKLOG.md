...
- 2026-06-27: Prepared the live WordPress `ADR Site Fixes` `119.7.0` patch: backend MetForm metadata for contact form `7487` removes the stale recaptcha field, inserts the backend `telephone` field, and keeps the frontend phone-preserver guard in sync with GH.io.
